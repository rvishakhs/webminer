    ...
    return (
        <Card>
            <CardHeader className='rounded-lg rounded-b-none border-b py-4 bg-gray-50 dark:bg-background'>    
                ...
                <CardDescription className='text-muted-foreground text-sm'>
                    Logs generated by the selected phase
                </CardDescription>
            </CardHeader>
                ...
                <TableBody>
                    {logs.map((log) => (
                        <TableRow key={log.id} className='text-muted-foreground '>
                            <TableCell width={190} className='text-xs text-muted-foreground p-[2px] pl-2'>{log.timestamp.toISOString()}</TableCell>
                            <TableCell 
                                width={80} 
                                className={cn("uppercase text-xs font-bold p-[3px] pl-2", (log.logLevel) === "error" && "text-destructive", 
                                (log.logLevel) === "warn" && "text-warning", (log.logLevel) === "info" && "text-primary")}
                            >
                                {log.logLevel}
                            </TableCell>
                            <TableCell width={190} className='text-xs text-muted-foreground p-[2px] pl-2'>{log.message}</TableCell>
                        </TableRow>
                    ))}                    
                </TableBody>
            </CardContent>
